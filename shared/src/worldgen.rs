...
impl HeightGrid {
    ...
    fn apply_beach_shelf(&mut self) {
        ...
        // ONLY the near-waterline strip (two 3x3 box passes) so the contour
        // itself becomes smooth at mesh scale; everything else keeps its
        // detail. Runs before the compression/step so the step lands on the
        // smoothed contour.
        ...
        for _ in 0..2 {
            ...
            for zi in 1..self.size - 1 {
                for xi in 1..self.size - 1 {
                    let idx = zi * self.size + xi;
                    let h = snapshot[idx];
                    if h <= SEA_LEVEL - 1.2 || h >= SEA_LEVEL + 1.2 {
                        continue;
                    }
                    let mut sum = 0.0;
                    for dz in 0..3 {
