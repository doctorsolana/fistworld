...
/// different version.
pub const WORLDGEN_VERSION: u32 = 12;
...
pub struct BiomeField {
    zone: Fbm<Perlin>,
    vein: Fbm<Perlin>,
    /// Broad within-biome quality fields. These deliberately use one octave:
    /// a farm on the next plot should resemble its neighbour, while farms on
    /// opposite sides of a settlement can still sit on noticeably different
    /// soil. Separate fields stop every profitable industry from clustering
    /// in the same magic patch.
    soil_quality: Perlin,
...
impl BiomeField {
    ...
    pub fn new_with_extent(seed: u64, half_extent: f32) -> Self {
        ...
        Self {
            ...
            // One smooth octave per resource is intentional. These are broad
            // regional gradients, not high-definition resource noise.
    ...
    /// Smooth seeded multiplier around 1.0. Perlin's output is bounded for
    /// these two-dimensional samples, but clamp defensively so a future noise
    /// implementation cannot create invalid resource values.
    fn regional_quality(noise: &Perlin, x: f32, z: f32, metres: f64, spread: f32) -> f32 {
        let patch = (noise.get([x as f64 / metres, z as f64 / metres]) as f32).clamp(-1.0, 1.0);
        1.0 + patch * spread
    }

    /// Effective resource availability at a point: the biome's base profile
    /// with broad seeded variation inside each biome, iron gated by veins
    /// (rare, concentrated), stone scaled by how rugged the ground actually
    /// is (rocky slopes and peaks quarry better than flat moor), and
    /// everything zeroed in the water.
    pub fn resources(&self, x: f32, z: f32, height: f32, slope: f32) -> ResourceProfile {
        ...
        // These wavelengths are intentionally larger than an ordinary village
        // plot and use no fine octaves. Quality changes gradually enough that
        // neighbouring fields agree, but a settlement can still discover a
        // richer edge and specialize around it.
        profile.farmland *= Self::regional_quality(&self.soil_quality, x, z, 520.0, 0.28);
        profile.wood *= Self::regional_quality(&self.timber_quality, x, z, 430.0, 0.24);
        let geology = Self::regional_quality(&self.geology_quality, x, z, 640.0, 0.24);
        profile.stone *= geology;
        profile.iron *= geology;
        ...
        profile.farmland = profile.farmland.clamp(0.0, 1.0);
        profile.wood = profile.wood.clamp(0.0, 1.0);
        profile.stone = profile.stone.clamp(0.0, 1.0);
        profile.iron = profile.iron.clamp(0.0, 1.0);
        profile
    }
}
...
mod tests {
    ...
    fn biomes_are_deterministic_and_iron_is_rare() {
        ...
        for zi in 0..80 {
            for xi in 0..80 {
                ...
                assert_eq!(r, b.resources(x, z, h, slope));
                iron_sum += r.iron;
                wood_sum += r.wood;
                if r.iron > 0.25 {
                    rich_iron_cells += 1;
    ...
    #[test]
    fn resource_quality_varies_smoothly_inside_each_biome() {
        let field = BiomeField::new_with_extent(2026, 4096.0);
        let mut meadow_farmland = Vec::new();
        let mut forest_timber = Vec::new();
        let mut highland_stone = Vec::new();

        // Hold terrain height and slope constant so this measures the new
        // within-biome fields rather than mountain, climate, or slope effects.
        for zi in 0..33 {
            for xi in 0..81 {
                let x = -3200.0 + xi as f32 * 80.0;
                let z = -1200.0 + zi as f32 * 75.0;
                let profile = field.resources(x, z, 8.0, 0.1);
                match field.biome(x, z, 8.0, 0.1) {
                    WorldBiome::Meadows => meadow_farmland.push(profile.farmland),
                    WorldBiome::Forest => forest_timber.push(profile.wood),
                    WorldBiome::Highlands => highland_stone.push(profile.stone),
                    _ => {}
                }
            }
        }
        ...
        assert!(meadow_farmland.len() > 100);
        assert!(forest_timber.len() > 100);
        assert!(highland_stone.len() > 20);
        assert!(
            spread(&meadow_farmland) > 0.08,
            "meadow soil stayed effectively constant"
        );
        assert!(
            spread(&forest_timber) > 0.08,
            "forest timber stayed effectively constant"
        );
        assert!(
            spread(&highland_stone) > 0.03,
            "highland geology stayed effectively constant"
        );

        // A plot-sized step remains a tiny change: the fields should read as
        // regions, not resource static or a checkerboard heat map.
        for (noise, metres, spread) in [
            (&field.soil_quality, 520.0, 0.28),
            (&field.timber_quality, 430.0, 0.24),
            (&field.geology_quality, 640.0, 0.24),
        ] {
            let here = BiomeField::regional_quality(noise, 417.0, -263.0, metres, spread);
            let next_plot = BiomeField::regional_quality(noise, 425.0, -263.0, metres, spread);
            assert!(
                (here - next_plot).abs() < 0.025,
                "quality changed too sharply across one plot: {here} -> {next_plot}"
            );
        }
    }
