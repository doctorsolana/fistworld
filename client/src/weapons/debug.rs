...
pub fn update_debug_overlay(
    ...
    mut counts_a: ParamSet<(
        ...
        Query<(), With<Bullet>>,
        Query<(), With<LocalTracer>>,
        Query<(), With<Vehicle>>,
        Query<(), With<crate::render::systems::SandParticle>>,
        Query<(), With<MuzzleSmoke>>,
    )>,
    mut counts_b: ParamSet<(
        Query<(), With<MuzzleFlash>>,
    ...
    if overlay_enabled.0 {
        ...
        *last_update = now;
        ...
        lines.push_str(&format!(
            ...
            counts_a.p5().iter().count(),
            counts_a.p6().iter().count(),
            collider_chunks.len(),
            collidable_props,
            baked_kinds,
            counts_a.p3().iter().count(),
            counts_a.p4().iter().count(),
            counts_a.p7().iter().count(),
            counts_b.p0().iter().count(),
            counts_b.p1().iter().count(),
            counts_b.p2().iter().count(),
            perf.1.p50_ms,
            perf.1.p95_ms,
            perf.1.p99_ms,
