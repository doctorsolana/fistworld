...
fn cavalry_musters_and_uses_the_normal_battalion_order_stream() {
    ...
    let battalion = world
        ...
        .0;
    assert_eq!(
        world.get::<SoldierRole>(battalion),
        Some(&SoldierRole::Cavalry)
    );
    // Clicking one member expands to both, exactly like infantry selection.
    let order = |command| UnitOrder {
    ...
    let (count, detail) = apply_unit_order(
        &mut world,
        "alice",
        order(UnitCommand::Move {
            target: Vec3::Z * 20.,
            frontage: None,
            mode: MovementMode::Move,
        }),
    ...
    let goal_a = world
        .get::<crate::player::orders::MarchOrder>(a)
        .unwrap()
        .destination;
    let goal_b = world
        .get::<crate::player::orders::MarchOrder>(b)
        .unwrap()
        .destination;
    ...
    assert_eq!(count, 2, "{detail}");
    assert!(world
        .get::<crate::player::combat::fronts::FormationMember>(a)
        .is_some());
...
fn cavalry_is_not_a_free_horse_supply_and_unmounted_recruits_cannot_join() {
    ...
    let cavalry_battalion = world
        .query::<(Entity, &Battalion, &SoldierRole)>()
        .iter(&world)
        .find(|(_, _, r)| **r == SoldierRole::Cavalry)
        .unwrap()
        .0;
    assert_eq!(
        apply_army_order(
...
fn public_equipment_and_mixed_membership_edits_cannot_discard_issued_horses() {
    ...
    let infantry = world
        .query::<(Entity, &Battalion, &SoldierRole)>()
        .iter(&world)
        .find(|(_, _, role)| **role == SoldierRole::Infantry)
        .unwrap()
        .0;
    assert_eq!(
        apply_army_order(
...
fn detached_cavalry_certifies_obstructed_attacks_with_a_mounted_formation_route() {
    ...
    app.world_mut().entity_mut(rider).insert(SkirmishOrder {
        target: Enemy::Person(target),
    });
    app.add_systems(Update, (rebuild_combat_space, steer_skirmishers).chain());
    app.update();
    assert!(
        app.world()
            .get::<crate::player::combat::DirectCombatApproach>(rider)
            .is_none(),
        "A foot-wide passage is not a clear mounted approach"
    );
    assert!(
        app.world().get::<MarchOrder>(rider).is_some(),
        "Blocked mounted attacks must enter the clearance-aware bounded planner"
    );
}
