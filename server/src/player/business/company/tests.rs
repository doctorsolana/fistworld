...
impl Fixture {
    ...
    fn order(&mut self, actor: Entity, action: HeroCompanyAction) -> Result<String, &'static str> {
...
fn company_governance_and_finance_need_no_live_or_operating_site() {
    for state in [
        ...
        .unwrap();
        f.order(f.master, HeroCompanyAction::DistributeAvailableProfit)
            .unwrap();
        ...
        assert_eq!(
            ...
            "command queues finance review; it cannot skip liability reserves"
        );
    }
}
...
fn capital_refusal_preserves_both_ledgers_and_coowners_cannot_be_silently_subsidized() {
    for (cash, contributed) in [(u64::MAX, 0), (900, u64::MAX)] {
        ...
        let account = *f.world.get::<CompanyAccount>(f.company).unwrap();
        assert!(
            f.order(f.master, HeroCompanyAction::ContributeCapital { amount: 1 })
                .is_err()
        );
