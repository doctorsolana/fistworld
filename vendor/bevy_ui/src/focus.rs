...
///
/// FISTWORLD VENDOR PATCH (the only change in this vendored crate): upstream
/// 0.19.0 stops the ancestor walk at the FIRST ancestor that does not clip,
/// so any interactive node separated from its scroll viewport by a plain
/// wrapper node was hit-tested UNCLIPPED — rows scrolled out of a list kept
/// their full click rects and stole clicks aimed at whatever was drawn over
/// them. The walk must continue to the root, applying each ancestor's clip
/// only where one exists (matching `update_clipping_system`, which renders
/// this way already).
